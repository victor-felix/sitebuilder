...
Redirect.isHome = function(homePath) {
  if (!homePath)
    return true;
  return typeof homePath === 'string' ? homePath == window.location.href : homePath.test(window.location.href);
}

Redirect.isExcluded = function(excludePath) {
  if (!excludePath)
    return false;
  return typeof excludePath === 'string' ? excludePath == window.location.href : excludePath.test(window.location.href);
}

function RedirectToMeuMobi(mobileUrl, homePath, excludePath) {
  try {
...
//RedirectToMeuMobi('http://m.meumobi.com', 'http://meumobi.com', 'http://meumobi.com/desktop');
