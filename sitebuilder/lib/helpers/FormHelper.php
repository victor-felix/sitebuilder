...
class FormHelper extends Helper {
	...
	public function create($action = null, $options = []) {
		$options += array(
			'method' => 'post',
	...
	public function close($submit = null, $attributes = []) {
	...
	public function multistring($name, $options = []) {
		...
		$values = isset($options['value']) && !empty($options['value'])
			? $options['value']
			: ['', '', '', ''];
		...
		foreach($values as $key => $value) {
			...
			$content .= $this->html->tag('input', '', $attr, true);
		}
	...
	public function input($name, $options = []) {
		...
		switch($options['type']):
			...
			case 'date':
				...
				break;
			case 'multistring':
				$input = $this->multistring($name, $options);
				break;
			case 'textarea':
				unset($options['type']);
