...
#
#
# Copyright (C) 2005 by Andreas Miller
...
class CSVHandler {
	...
	var $DataKey;
	var $HeaderData;	//
	var $HeaderExtendedData; // orginal more distance and index fields
	var $ItemsList;	//
	var $Items_Count;
	var $color;

// Standard User functions
	function CSVHandler($Filename, $Separator, $KeyFieldName="index") {		//Constructor
	...
	function ReadCSV() {			//read data into this->ItemsList and return it in an array 
		...
		$this->HeaderExtendedData= $this->HeaderData;
		array_push($this->HeaderExtendedData, "index");
		while ($DataLine = fgetcsv ($fp, 3000, $this->Separator)) {
			for($i=0;$i<count($this->HeaderExtendedData);$i++){
				$Item[$this->HeaderExtendedData[$i]]=isset($DataLine[$i])?$DataLine[$i]:$this->Items_Count;
			}
	...
	function getDistance($f, $s) {
		$distance = sqrt(pow($f['lat']-$s['lat'],2)+pow($f['long']-$s['long'],2));
		return $distance;
	}
	
	function SelectNearest($lat=null, $long=null) {
		$Data= $this->ReadCSV();
		array_push($this->HeaderExtendedData, "distance");
		$result= array();
		while(list($key,$line)=each($this->ItemsList)) {
			$distance= $this->getDistance(array("lat"=>$lat, "long"=>$long), array("lat"=>$line["latitude"], "long"=>$line["longitude"]));
			if ($distance<0.5) {
	...
	function exportCsv($items=null) {
		...
		$HData="";
		while(list($LineKey,$LineVal)=each($items)) {	//Read Data Lines
			$HDataLine="";
			while(list($DataKey,$DataVal)=each($LineVal)) {			//Dissect one Data Line
				if (in_array($DataKey, $this->HeaderData))
					$HDataLine.=$DataVal.$this->Separator;
			}
			$HData.=substr($HDataLine, 0, strlen($HDataLine)-1);
			$HData.="\n";
		}
		print ($HData);
	}

	function ListAll($items=null) {					//prints a list of all Data
		if(!isset($items)){
			$Data=$this->ReadCSV();
			reset ($this->ItemsList);
			$items=$this->ItemsList;
